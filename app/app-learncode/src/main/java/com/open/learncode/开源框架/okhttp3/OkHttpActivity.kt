    ...
    fun okhttpSyncTest() {
        ...
        Thread(Runnable {
            try {
                ...
                Log.d(
                    TAG,
                    "同步请求成功=》当前线程：id=${Thread.currentThread().id}，name=${Thread.currentThread().name}"
                )
                // 注意子线程中不能操作更新UI
                runOnUiThread {
                    //Toast.makeText(this, data, Toast.LENGTH_SHORT).show()
                    tv_okhttp_text.text = "同步请求成功=》$data"
                }
            } catch (e: IOException) {
                e.printStackTrace()
                ...
                Log.d(
                    TAG,
                    "同步请求失败=》当前线程：id=${Thread.currentThread().id}，name=${Thread.currentThread().name}"
                )
                tv_okhttp_text.text = "同步请求失败=》${e.toString()}"
            }
        }).start()
    }
    ...
    fun okhttpAsyncTest() {
        ...
        call.enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                Log.d(
                    TAG,
                    "异步请求成功=》当前线程：id=${Thread.currentThread().id}，name=${Thread.currentThread().name}"
                )
                // 注意子线程中不能操作更新UI
                runOnUiThread {
                    //Toast.makeText(mContext, data, Toast.LENGTH_SHORT).show()
                    tv_okhttp_text.text = "异步请求成功=》$data"
                }
            }
            ...
            override fun onFailure(call: Call, e: IOException) {
                ...
                Log.d(
                    TAG,
                    "异步请求失败=》当前线程：id=${Thread.currentThread().id}，name=${Thread.currentThread().name}"
                )
                tv_okhttp_text.text = "异步请求失败=》${e.toString()}"
            }
    ...
    fun okhttpTest() {
        ...
        call.enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                tv_okhttp_text.text = "请求成功"
            }

            override fun onFailure(call: Call, e: IOException) {
                Log.d("TAG", "onFailure: ")
                Log.d("TAG", "异步请求失败=》当前线程：" + Thread.currentThread().name)
                tv_okhttp_text.text = "请求失败"
            }
        })
    }
