package com.open.learncode.算法.old.wcopy.leetcode_剑指offer.A5_跟数组相关的算法;
...
public class JZ41_数据流的中位数 {

    // 小顶堆，保存较大的一半(排序：入队->出队 大->小）
    private static Queue<Integer> A = new PriorityQueue<>();
    // 大顶堆，保存较小的一半(排序：出队->入队 大->小）
    private static Queue<Integer> B = new PriorityQueue<Integer>(new Comparator<Integer>() {
        @Override
