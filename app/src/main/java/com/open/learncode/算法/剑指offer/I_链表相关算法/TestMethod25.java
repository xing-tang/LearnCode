package com.open.learncode.算法.剑指offer.I_链表相关算法;
...
//        PrintUtils.getInstance().printListNode(method_2(node1, node2), "打印合并后的链表");
    }
     ...
     * @return 返回合并后的链表头节点
     */
    private static ListNode<Integer> method_2(ListNode<Integer> head1, ListNode<Integer> head2) {
        if (head1 == null) {
            return head2;
        } else if (head2 == null) {
            return head1;
            ...
            head2.next = method_2(head1, head2.next);
            return head2;
        }
    }
}